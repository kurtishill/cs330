...
is_set :- use_module(library(lists)).
guitracer :- use_module(library(gui_tracer)).
...
spot(_, _, _, 9).
spot(Row, Board, RowNum, ColNum) :-
	columnAsList(Board, ColNum, Col),
	cubeNum(RowNum, ColNum, CubeNum),
	getCube(Board, CubeNum, Cube),
  nth0(ColNum, Row, Spot),
	(nonvar(Spot);
	 var(Spot),
 	 digit(Spot),
	 is_set(Row),
	...
	spot(Row, Board, RowNum, NewColNum).


loop([], _, 9, _).
loop([Row|Rest], Board, RowNum, ColNum) :-
	spot(Row, Board, RowNum, ColNum),
	NewRowNum is RowNum + 1,
	loop(Rest, Board, NewRowNum, 0).

solve(Board) :-

	loop(Board, Board, 0, 0).
